...
const STYLE_CONFIG = {
  'ИНФОРМИРОВАНИЕ': {
    ...
    summary: `
      <p>Ваш результат: <br/>Директивный стиль руководства<br/>У вас ярко выражен директивный (инструктирующий) стиль руководства.</p>
      <p>Вы склонны самостоятельно определять цели и пути их достижения, чётко формулируете задачи и требуете их исполнения в соответствии с установленными стандартами.</p>
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          'Директивный стиль наилучшим образом подходит для работы с сотрудниками уровня зрелости R1 — “Начинающий энтузиаст”, то есть: новыми или неопытными работниками, не обладающими необходимыми навыками для выполнения задач, но проявляющими высокий уровень мотивации и энтузиазма.',
          ...
          'Руководитель в этой ситуации должен быть для них наставником, помогая освоить основы и не допустить ошибок.'
        ]
      },
      {
  ...
  'ОБУЧЕНИЕ': {
    ...
    title: 'ОБУЧЕНИЕ',
    summary: `
      <p>Ваш результат: Обучающий стиль руководства<br/> У вас ярко выражен обучающий (коучинговый) стиль руководства.</p>
      <p>Вы склонны сочетать высокую директивность с активной поддержкой. С одной стороны, вы ясно формулируете задачи и сохраняете ответственность за принятие решений. С другой — вовлекаете сотрудников в обсуждение, объясняете логику действий и помогаете им понять «почему» и «зачем» выполняется то или иное задание.</p>
    ...
    sections: [
      ...
      },
      {
        title: 'Когда этот стиль наиболее эффективен',
        items: [
          'Обучающий стиль наиболее результативен при работе с сотрудниками уровня зрелости R2 — "Разочарованный ученик":',
          ...
          'В такой ситуации руководство должен проявлять терпение, помогать сотруднику восстанавливать уверенность, учить и направлять, не снижая при этом требований.'
        ]
      },
      {
  ...
  'ПОДДЕРЖКА': {
    ...
    title: 'ПОДДЕРЖКА',
    summary: `
      <p>Ваш результат:<br/>Обучающий стиль руководства<br/>
      У вас выражен поддерживающий (совместный) стиль руководства.</p>
      <p>Вы стремитесь к партнёрским отношениям с сотрудниками, активно вовлекаете их в процесс принятия решений, предоставляете свободу действий и одновременно поддерживаете морально</p>
    ...
    sections: [
      ...
      },
      {
        title: 'Когда этот стиль наиболее эффективен',
        items: [
          'Поддерживающий стиль управления оптимален для работы с сотрудниками уровня зрелости R3 — “Осторожный исполнитель”, которые:',
          ...
          'Руководитель здесь играет роль наставника-партнера — помогает сотруднику поверить в свои силы, укрепить внутреннюю мотивацию и развить автономность.'
        ]
      },
      {
  ...
  'ДЕЛЕГИРОВАНИЕ': {
    ...
    summary: `
      ...
      <p><em>У вас слабо выражен делегирующий (доверительный) стиль руководства.</em></p><br/>
    ...
    sections: [
      ...
      },
      {
        title: 'Когда этот стиль наиболее эффективен',
        items: [
          'Делегирующий стиль подходит для сотрудников уровня зрелости R4 — “Уверенный профессионал”, которые:',
          ...
          'Для таких сотрудников руководитель становится партнёром и стратегом, создающим условия для профессионального роста и самореализации.'
        ]
      },
      {
...
export default function ResultPage() {
  ...
  const hasClearLeader = sortedScores.length >= 2
    ? sortedScores[0] - sortedScores[1] > 5
    : true;

  if (!hasClearLeader || maxScore === 0 || leaders.length > 1) {
    return (
  ...
  return (
      ...
      <div className={styles.topSection}>
        ...
        <div className={styles.summary}>
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: mainStyle.summary }}
          />
        </div>
      </div>
    ...
    <div className={styles.actions}>
      <button
        ...
        onClick={() => {
          if (result.actions.result_url) {
            navigator.clipboard.writeText(result.actions.result_url);
